...
const transfer = async (req, res, next) => {
  ...
  try {
    ...
    // 3- add transaction in sending account 
    sendingAccount.transactions.push({
      change: -1 * +balanceTransfered ,
      balance: sendingAccount.balance 
    }) ;
    sendingAccount.markModified('transactions');
    // 4- add transaction in receiving account 
    receivingAccount.transactions.push({
      change: +balanceTransfered ,
      balance: sendingAccount.balance 
    }) ;
    receivingAccount.markModified('transactions');
