...
import { getAllAccounts , resetAccoutActions} from "../../state/features/Admin/AccountActions/AccountActionsSlice";
import { AccountsListControl } from "../../components/admin/AccountsList";
...
export default function AdminDashboard() {
  ...
  const {accountsList} = useSelector((state) => state.accountActions)
    ...
    <div className="min-h-screen  flex flex-nowrap">
      ...
      <div className="overflow-x-auto w-full h-full flex justify-center items-center">
        <div className="w-full min-h-screen flex  items-center flex-col gap-6 p-3 md:p-6 bg-slate-50">
          ...
          {/* users Account Request*/}
          {activeTab === "Accounts" && (
            <AccountsListControl accountsList={accountsList} />
          )}
        </div>
      </div>
    </div>
