...
import { addAllInterest, resetAccoutActions } from "../../state/features/Admin/AccountActions/AccountActionsSlice";
...
export const AccountsListControl = ({ accountsList }) => {
  ...
  //filtered users list
  const filteredUsers = accountsList ;
    // accountsList &&
    // accountsList.filter((user) => {
    //   if (
  ...
  const handleCalculate = (e, accountId) => {
    e.preventDefault();
    ...
    dispatch(addAllInterest(accountId));
  };
    ...
    return (
      ...
      >
        ...
        <th
          ...
          className="p-2 text-gray-900 whitespace-nowrap  border-x-2 text-center"
        >
          <form onSubmit={(event) => handleCalculate(event, user._id)}>
            <FormButton
              text={{ default: "Calculate" }}
              bgColor={["bg-green-600", "bg-green-700", "bg-green-800"]}
            />
          </form>
        </th>
  ...
  //clean up for accountsList status (on mount , unmount)
  UseResetStatus(() => {
    dispatch(resetAccoutActions());
  });
  ...
  return (
    <div className="max-w-5xl w-full">
      <h3 className="text-2xl my-10 p-3 text-center font-bold bg-blue-200 text-gray-900 border-b-4 border-blue-800 rounded shadow">
        Accounts List ({filteredUsers && filteredUsers.length})
      </h3>

      {/*search users with name*/}
      {(accountsList.length !== 0 || isLoading) && (
        <div className="flex justify-center items-center flex-wrap md:flex-nowrap gap-4 mb-6 p-4 bg-blue-200 rounded-md border-b-4 border-blue-800">
          ...
          >
            <FcSearch size={40} /> <span>Search by Account ID:-</span>
          </label>
